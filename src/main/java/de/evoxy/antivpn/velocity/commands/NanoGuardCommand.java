package de.evoxy.antivpn.velocity.commands;
...
public class NanoGuardCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
        ...
        if(args.length == 1){
            if(args[0].equalsIgnoreCase("notifications") || args[0].equalsIgnoreCase("notify")){
                ...
                NanoGuardMain.getInstance().getConfigConfigManager().save();
                ...
                if(defaultConfig.database.enabled){
                    ...
                    List<BlockedAddressesTable> blockedAddresses = new Query<>(BlockedAddressesTable.class, NanoGuardMain.getInstance().getDataStore()).execute();
    ...
    public List<String> suggest(Invocation invocation) {
        ...
        if(args.length == 1){
            ...
            new Query<>(WhitelistedTable.class, NanoGuardMain.getInstance().getDataStore()).execute().forEach(entry -> suggestions.add(entry.value));
        }
    ...
    public boolean hasPermission(Invocation invocation) {
        return invocation.source().hasPermission(NanoGuardMain.getInstance().getDefaultConfig().permissions.command);
    }
}
